...
## Glossary Text Lookup (Source/Target Language Direction)

- Added backend lookup API in `scripts/glossary_manager.py`:
  - new `lookup` subcommand + `lookup_text(...)`
  - query by arbitrary input text
  - returns matched entries with `source_lang`, `target_lang`, `language_pair`, and `matched_in` (`source`/`target`).
- Added Tauri command `lookup_glossary_text` in `tauri-app/src-tauri/src/lib.rs` and registered it in `invoke_handler`.
- Added frontend bindings in `tauri-app/src/lib/tauri.ts`:
  - `GlossaryLookupItem`
  - `GlossaryLookupResult`
  - `lookupGlossaryText(...)`.
- Added new Glossary page section in `tauri-app/src/pages/Glossary.tsx`:
  - input text lookup bar
  - result table showing company, `source_lang -> target_lang`, matched side, source text, target text.
- Added tests:
  - `tests/test_glossary_manager.py` for lookup source/target matching and language direction.
- Validation:
  - Python tests passed
  - `cargo check` passed
  - `pnpm build` passed

## Glossary False-Positive Fix (`تقويم` vs `قوي`)

- Root cause: glossary enforcer used raw substring matching (`ar_norm in src_norm`), causing false positives where short term `قوي` matched inside unrelated word `تقويم`.
- Fixes:
  - Added `contains_arabic_term(...)` in `scripts/kb_glossary_enforcer.py` using letter-boundary matching (not substring).
  - Updated `select_terms_for_sources(...)` to use boundary-aware matching.
  - Updated orchestrator glossary validation (`_validate_glossary_enforcer`) to use boundary-aware matching.
  - Updated suffix-cleanup guard (`_strip_redundant_glossary_suffixes`) to use boundary-aware matching.
- Added regression tests:
  - `tests/test_kb_glossary_enforcer.py`:
    - `contains_arabic_term` avoids `تقويم`→`قوي` false hit.
    - `select_terms_for_sources` avoids false positive.
  - `tests/test_glossary_enforcer_gate.py`:
    - enforcer avoids false positive in unit-level validation.
- Verification:
  - `.venv/bin/python -m unittest -v tests/test_kb_glossary_enforcer.py tests/test_glossary_enforcer_gate.py tests/test_openclaw_translation_orchestrator.py` passed (`34` tests).
  - Restarted worker (`scripts/start.sh --restart-worker`) to apply runtime fix.
