...
## Manual Fix (Needs-Attention Job) - Untranslated Question Column

- Job: `job_telegram_20260218_154138_5b828867` (لواء الرصيفة Field Directorate)
- Symptom: Column C answers were translated, but Column B question prompts remained Arabic (24 cells per file; 48 total across the duplicate XLSX), causing `xlsx_translation_map_incomplete:missing=48`.
- Fix (output): Patched both output XLSX files in-place and created backups (`*_before_manual_fix.xlsx`). Verified `arabic_cells_remaining: 0`.
- Fix (pipeline): Updated `scripts/openclaw_translation_orchestrator.py` to:
  - Strengthen SPREADSHEET_TRANSLATION instructions to translate all `format_preserve.xlsx_sources[].cell_units` (even “auto-populates” question text).
  - Include `xlsx_missing_units_sample` (with source text) in hard-gate retry hints so Codex can converge when preserve coverage is incomplete.
